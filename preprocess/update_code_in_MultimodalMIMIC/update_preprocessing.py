...
sys.path.append('C:/code/MIMIC3/mimic3-benchmarks-master')
...
from MultimodalMIMIC_main.text_utils import *
...
def get_start_time_diffs(times, st):
    start2time = []
    difftimes = []
    st = np.datetime64(st)
    for t in times:
        time = np.datetime64(t)
        dt = diff_float(st, time)
        difftimes.append(dt)
    start2time.append(difftimes)
    return start2time


def extract_irregular(dataPath_in,dataPath_out):
    # Opening JSON file
    channel_info_file = open(os.path.join(os.path.dirname(__file__), './Data/irregular/channel_info.json'))
    dis_config_file=open(os.path.join(os.path.dirname(__file__), './Data/irregular/discretizer_config.json'))
...
def merge_text_ts(textdict, timedict, start_times,tslist,period_length,dataPath_out,mergePath_out):
    suceed = 0
    missing = 0
    ts_note_list = []
    for idx, ts_dict in enumerate(tslist):
        ...
        if name in textdict:
            ...
            if period_length is not None:  # 48ihm
                ts_dict['text_time_to_end']= get_time_to_end_diffs(timedict[name], start_times[name],endtime=period_length+1)[0]

            ts_dict['text_time_to_start'] = get_start_time_diffs(timedict[name], start_times[name])
            suceed += 1
            # ts和note都有的才加入
            ts_note_list.append(ts_dict)
        else:
            missing += 1
    ...
    with open(mergePath_out, 'wb') as f:
        pickle.dump(ts_note_list, f)
    return 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data', type=str, help='Path to the data of task',
                        default=os.path.join(os.path.dirname(__file__), '../data/in-hospital-mortality/'))  # '../data/in-hospital-mortality/'  '../data/phenotyping/'
    parser.add_argument("--period_length", default=48, type=int, help="period length of reader.") # 48 24
    parser.add_argument("--task", default='ihm', type=str, help="task name to create data")  # ihm pheno
    parser.add_argument("--outputdir", default='./Data/', type=str, help="data output dir")
    ...
    if args.task=='ihm':
        ...
        train_reader = InHospitalMortalityReader(dataset_dir=os.path.join(args.data, 'train'),
                                                listfile=os.path.join(args.data, 'train_listfile.csv'),  # train_listfile_2.csv
                                                period_length=args.period_length)
        # val_reader = InHospitalMortalityReader(dataset_dir=os.path.join(args.data, 'train'),
        #                                     listfile=os.path.join(args.data, 'val_listfile_2.csv'),
        #                                     period_length=args.period_length)
    ...
    if args.task == "ihm":
        ...
        normalizer_state = '../mimic3models/phenotyping/ph_ts{}.input_str-{}.start_time-zero.normalizer'.format(
            args.timestep, args.imputation)
    normalizer_state = os.path.abspath(os.path.join(os.path.dirname(__file__), normalizer_state))
    ...
    textdata_fixed = os.path.join(os.path.dirname(__file__), "../data/root/train_text_fixed/")
    starttime_path = os.path.join(os.path.dirname(__file__), "../data/train_starttime.pkl")
    test_textdata_fixed = os.path.join(os.path.dirname(__file__), "../data/root/test_text_fixed/")
    test_starttime_path = os.path.join(os.path.dirname(__file__), "../data/test_starttime.pkl")
    ...
    for mode in ['train', 'test']:  # 'val'
        ...
        merge_text_ts(data_text, data_times, data_time,tsdata, args.period_length,output_dir+mode+'p2x_data.pkl', output_dir+mode+'ts_note_data.pkl')
